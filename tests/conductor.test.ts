...
describe('routeConductorMessage (stub)', () => {
  test('@agent-id prefix routes straight to that agent and strips the prefix', async () => {
    ...
    const res = await routeConductorMessage(db, realAgents, '@meta-ads-auditor what is pipeline?');
    expect(res.routedTo).toBe('meta-ads-auditor');
    expect(res.reply.length).toBeGreaterThan(0);
    expect(db.agentMessages.byAgent('meta-ads-auditor')[0].content).toBe('what is pipeline?');
  });
  ...
  test('routes through the conductor and returns routedTo + reply', async () => {
    ...
    const res = await POST(
      new Request('http://localhost/api/agents/conductor/chat', {
        method: 'POST',
        body: JSON.stringify({ message: '@meta-ads-auditor how are deals?' }),
      }),
      { params: { id: 'conductor' } },
    );
    expect(res.status).toBe(200);
    const body = await res.json();
    expect(body.routedTo).toBe('meta-ads-auditor');
