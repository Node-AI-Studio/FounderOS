...
// Everything reads bright at rest (2026-07-12: "keep it all lit up"
...
// ── operator memory core (Notes constellation) ────────────────────────────
...
// The whole vault burns one HARD reddish orange (design call, 2026-07-06)
...
export function KnowledgeGraph({
  ...
  /** distilled brain-store constellation drawn at the core (the operator = their memory) */
  memory?: MemoryGraph;
  ...
  // departments in EXPANDED form (design note): the flanks carry their whole tree
  ...
  // large wheel instead of springing (design note: "a large wheel animation").
  ...
  function configure(sim: Simulation<SimNode, undefined>) {
    ...
    // the rim (design note: a huge wheel with the departments ALREADY expanded):
  ...
  // lenses (2026-07-12 taxonomy): slice the graph by entity type, business
  ...
  const clearAll = () => {
    ...
    clearDetail();
    // GLIDE back to the main view (2026-07-12: "see the animation of
  ...
  const onNodeClick = (n: KGNode) => {
    if (n.kind === 'self') {
      // The operator IS the memory: clicking them dives into (or out of) the
  ...
  // The unprompted pop-up read as a bug (2026-07-12). Cards now open
  ...
  const graphInner = (
    <>
      ...
      >
        ...
        {/* the flank departments ride the rim ALREADY EXPANDED (design note): their
        ...
        {nodes.map((n) => {
          ...
          // the tree (design note: nothing behind the pillar I'm looking at),
          // every other unfocused node rides the carousel fully hidden
          // flanks show a PORTION of their department (design note): the gateway
          ...
          // The operator rendered as their memory: the Notes constellation of real
